...
class Local_Feature_Processing(nn.Module):
    def __init__(self, channels, out_channels,  blocks=1, groups=1, res_expansion=1, bias=True,
        ...
        for _ in range(blocks):
            operation1.append(
                Efficient_KAN(in_features = out_channels, hidden_features = out_channels,out_features=out_channels )
            )
    ...
    def forward(self, x, center):
        ...
        x = x.permute(0, 2, 1)
        x = x + self.operation1(self.norm1(x)) 
        x = x.permute(0, 2, 1)
        x = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)
        x = x.reshape(b, n, -1).permute(0, 2, 1)
        center = self.transfer2(center)
        center = center.permute(0,2,1)
        center = center + self.operation2(self.norm2(center))
        center = center.permute(0,2,1)
        x = x + center
        return x
...
class PointKANElite(nn.Module):
    def __init__(self, num_classes=50, points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(PointKANElite, self).__init__()
...
def pointKANElite(num_classes=50, **kwargs) -> PointKANElite:
    return PointKANElite(num_classes=num_classes, points=2048, embed_dim=32, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="anchor",
