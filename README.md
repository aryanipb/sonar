...
### AdaPoinTr Inference (System-Compatible Pipeline)
For a complete beginner-friendly, step-by-step guide (download -> setup -> inference -> interactive 3D view), see [README_ADAPOINTR_INFERENCE.md](./README_ADAPOINTR_INFERENCE.md).


If your environment cannot build all CUDA extensions (common on newer CUDA/toolchains), use the compatibility pipeline below. It keeps inference working without hard pinning old CUDA/PyTorch versions.
...
Notes:
- `tools/check_inference_env.py` reports your runtime package/toolchain compatibility.
- `chamfer_dist` is optional for inference (still required for training/evaluation losses).
- `pointnet2_ops` has a local PyTorch fallback for compatibility when the external CUDA build is unavailable.
