...
def get_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Skip samples that already have an output fine.npy.')
...
def inference_single(model, pc_path, args, config, root=None):
    ...
    if args.out_pc_root != '':
        if root is None:
            rel_path = os.path.splitext(os.path.basename(pc_path))[0]
        else:
            rel_path = os.path.splitext(pc_path)[0]
        target_path = os.path.join(args.out_pc_root, rel_path)
        if args.skip_existing and os.path.exists(os.path.join(target_path, 'fine.npy')):
            print(f'[Skip] {pc_path} (already exists)')
            return
...
def main():
    ...
    if args.pc_root != '':
        supported_ext = {'.npy', '.pcd', '.ply', '.h5', '.txt'}
        pc_file_list = sorted(
            f for f in os.listdir(args.pc_root)
            if os.path.isfile(os.path.join(args.pc_root, f)) and os.path.splitext(f)[1].lower() in supported_ext
        )
        if not pc_file_list:
            raise RuntimeError(f'No supported point cloud files found under {args.pc_root}')
